...
import { ExternalLink, Trash2, Edit, CheckCircle2, Github } from "lucide-react";
...
const projectTypeColor: Record<string, string> = {
  ...
  Fullstack: "bg-purple-500/10 text-purple-400 border border-purple-500/20",
  Mobile: "bg-orange-500/10 text-orange-400 border border-orange-500/20",
  DevOps: "bg-yellow-500/10 text-yellow-400 border border-yellow-500/20",
  CLI: "bg-gray-500/10 text-gray-400 border border-gray-500/20",
...
export default function DevTable({ entries, onRefresh, onEdit }: Props) {
  ...
  if (!entries.length) return (
    <motion.div className="p-8 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <h3 className="text-lg font-medium mb-2">No dev projects yet</h3>
      <p className="text-muted-foreground">Start tracking your development projects.</p>
    </motion.div>
  );
          ...
          <tbody>
            {entries.map((entry, i) => (
              <motion.tr key={entry.id} className="leetcode-hover" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: i * 0.05 }}>
                <td><div className="flex items-center justify-center"><CheckCircle2 className="w-4 h-4 text-blue-400" /></div></td>
                <td>
                  <div className="flex items-center gap-3">
                    <span className="text-muted-foreground text-sm">{i + 1}.</span>
                    <div>
                      <div className="font-medium text-foreground">{entry.title}</div>
                      {entry.description && <div className="text-xs text-muted-foreground mt-0.5 max-w-xs truncate">{entry.description}</div>}
                    </div>
                  </div>
                </td>
                <td>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${projectTypeColor[entry.projectType] || projectTypeColor.Other}`}>
                    {entry.projectType || "—"}
                  </span>
                </td>
                <td>
                  <div className="flex flex-wrap gap-1">
                    {entry.frontendTech && <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-blue-500/10 text-blue-300 border border-blue-500/20">{entry.frontendTech.split(',')[0].trim()}</span>}
                    {entry.backendTech && <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-green-500/10 text-green-300 border border-green-500/20">{entry.backendTech.split(',')[0].trim()}</span>}
                    {entry.databaseTech && <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-amber-500/10 text-amber-300 border border-amber-500/20">{entry.databaseTech.split(',')[0].trim()}</span>}
                  </div>
                </td>
                ...
                <td>
                  <div className="flex items-center gap-2">
                    {entry.githubUrl && (
                      <motion.a href={entry.githubUrl} target="_blank" rel="noopener noreferrer" className="p-1 rounded hover:bg-muted/50" whileHover={{ scale: 1.2 }}>
                        <Github className="w-4 h-4 text-muted-foreground hover:text-foreground" />
                      </motion.a>
                    )}
                    {entry.liveUrl && (
                      <motion.a href={entry.liveUrl} target="_blank" rel="noopener noreferrer" className="p-1 rounded hover:bg-muted/50" whileHover={{ scale: 1.2 }}>
