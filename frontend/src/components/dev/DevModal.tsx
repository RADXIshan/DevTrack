...
export default function DevModal({ isOpen, onClose, onAdd, initialData }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    databaseTech: "", otherTech: "", githubUrl: "", liveUrl: "", description: "",
  });
  ...
  useEffect(() => {
    if (isOpen) {
      setFormData(initialData ? {
        ...
        frontendTech: initialData.frontendTech || "",
        backendTech: initialData.backendTech || "",
        databaseTech: initialData.databaseTech || "",
        otherTech: initialData.otherTech || "",
        githubUrl: initialData.githubUrl || "",
        liveUrl: initialData.liveUrl || "",
        description: initialData.description || "",
      } : { title: "", projectType: "Fullstack", frontendTech: "", backendTech: "", databaseTech: "", otherTech: "", githubUrl: "", liveUrl: "", description: "" });
    }
  }, [initialData, isOpen]);
        ...
        <motion.div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <motion.div initial={{ opacity: 0, scale: 0.9, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 30 }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="bg-background border border-border rounded-lg w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h2 className="text-xl font-semibold text-foreground">{initialData ? "Edit" : "Add"} Dev Project</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto max-h-[calc(90vh-140px)] custom-scrollbar">
              <div>
                ...
                <input type="text" required value={formData.title} onChange={e => set("title", e.target.value)} className={inputClass} placeholder="E-commerce Platform, Chat App, Portfolio..." />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className={labelClass}>Project Type</label>
                  <select value={formData.projectType} onChange={e => set("projectType", e.target.value)} className={inputClass}>
                ...
                <div>
                  ...
                  <input type="url" value={formData.githubUrl} onChange={e => set("githubUrl", e.target.value)} className={inputClass} placeholder="https://github.com/..." />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.frontendTech} onChange={e => set("frontendTech", e.target.value)} className={inputClass} placeholder="React, Vue, Next.js, TypeScript..." />
                </div>
                <div>
                  <label className={labelClass}>Backend Technologies</label>
                  <input type="text" value={formData.backendTech} onChange={e => set("backendTech", e.target.value)} className={inputClass} placeholder="Node.js, Express, Django, FastAPI..." />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.databaseTech} onChange={e => set("databaseTech", e.target.value)} className={inputClass} placeholder="PostgreSQL, MongoDB, Redis, MySQL..." />
                </div>
                <div>
                  <label className={labelClass}>Other Tech / Tools</label>
                  <input type="text" value={formData.otherTech} onChange={e => set("otherTech", e.target.value)} className={inputClass} placeholder="Docker, AWS, CI/CD, Prisma..." />
                </div>
              </div>

              <div>
                <label className={labelClass}>Live URL</label>
                <input type="url" value={formData.liveUrl} onChange={e => set("liveUrl", e.target.value)} className={inputClass} placeholder="https://myproject.com" />
              </div>

              <div>
                <label className={labelClass}>Description & Notes</label>
                <textarea value={formData.description} onChange={e => set("description", e.target.value)} rows={4} className={`${inputClass} resize-none`} placeholder="Architecture decisions, challenges faced, lessons learned..." />
              </div>
            </form>
