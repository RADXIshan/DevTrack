...
const designTypeBadge: Record<string, string> = {
  HLD: "bg-orange-500/10 text-orange-400 border border-orange-500/20",
  LLD: "bg-teal-500/10 text-teal-400 border border-teal-500/20",
  Both: "bg-violet-500/10 text-violet-400 border border-violet-500/20",
  Concepts: "bg-slate-500/10 text-slate-400 border border-slate-500/20",
};

export default function SystemDesignTable({ entries, onRefresh, onEdit }: Props) {
  ...
  const handleDelete = async () => {
    if (!toDelete) return;
    try {
      await axios.delete(`${API_BASE_URL}/api/systemdesign/${toDelete}`, { headers: { Authorization: `Bearer ${user?.token}` } });
      setToDelete(null); onRefresh();
      toast("success", "System design entry deleted.");
  ...
  if (!entries.length) return (
    <motion.div className="p-8 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <h3 className="text-lg font-medium mb-2">No system design entries yet</h3>
      <p className="text-muted-foreground">Start tracking your system design practice.</p>
    </motion.div>
  );
          ...
          <tbody>
            {entries.map((entry, i) => (
              <motion.tr key={entry.id} className="leetcode-hover" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: i * 0.05 }}>
                <td><div className="flex items-center justify-center"><CheckCircle2 className="w-4 h-4 text-orange-400" /></div></td>
                <td>
                  <div className="flex items-center gap-3">
                    <span className="text-muted-foreground text-sm">{i + 1}.</span>
                    <div>
                      <div className="font-medium text-foreground">{entry.title}</div>
                      {entry.notes && <div className="text-xs text-muted-foreground mt-0.5 max-w-xs truncate">{entry.notes}</div>}
                    </div>
                  </div>
                </td>
                <td>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${designTypeBadge[entry.designType] || designTypeBadge.HLD}`}>
                    {entry.designType || "HLD"}
                  </span>
                </td>
                <td><span className="text-sm text-muted-foreground max-w-[160px] truncate block">{entry.patterns || "—"}</span></td>
                <td><span className="text-sm text-muted-foreground max-w-[160px] truncate block">{entry.components || "—"}</span></td>
                <td><span className="text-sm text-muted-foreground">{format(new Date(entry.completedAt), "MMM d, yyyy")}</span></td>
                <td>
                  <div className="flex items-center gap-2">
                    {entry.resourcesUrl && (
                      <motion.a href={entry.resourcesUrl} target="_blank" rel="noopener noreferrer" className="p-1 rounded hover:bg-muted/50" whileHover={{ scale: 1.2 }}>
