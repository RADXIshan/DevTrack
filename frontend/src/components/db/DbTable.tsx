...
const dbTypeBadge: Record<string, string> = {
  SQL: "bg-blue-500/10 text-blue-400 border border-blue-500/20",
  NoSQL: "bg-green-500/10 text-green-400 border border-green-500/20",
  VectorDB: "bg-purple-500/10 text-purple-400 border border-purple-500/20",
};

export default function DbTable({ entries, onRefresh, onEdit }: Props) {
  ...
  const handleDelete = async () => {
    if (!toDelete) return;
    try {
      await axios.delete(`${API_BASE_URL}/api/db/${toDelete}`, { headers: { Authorization: `Bearer ${user?.token}` } });
      setToDelete(null); onRefresh();
      toast("success", "Database entry deleted.");
  ...
  if (!entries.length) return (
    <motion.div className="p-8 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <h3 className="text-lg font-medium mb-2">No database entries yet</h3>
      <p className="text-muted-foreground">Start tracking your database learning journey.</p>
    </motion.div>
  );
                ...
                <td>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {entry.useCase && <div className="text-xs text-muted-foreground mt-0.5 max-w-xs truncate">{entry.useCase}</div>}
                    </div>
                  </div>
                </td>
                <td>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${dbTypeBadge[entry.dbType] || dbTypeBadge.SQL}`}>
                    {entry.dbType || "SQL"}
                  </span>
                </td>
                <td>
                  {entry.dbTechnology ? (
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-slate-500/10 text-slate-300 border border-slate-500/20">{entry.dbTechnology}</span>
                  ) : <span className="text-muted-foreground text-sm">—</span>}
                </td>
                <td><span className="text-sm text-muted-foreground max-w-xs truncate block">{entry.conceptsCovered || "—"}</span></td>
